...
- **Never re-add `cleanUrls` to `vercel.json`.** It 308-redirects `/index.html` → `/`, which makes the `/(.*)` → `/index.html` SPA rewrite point at a non-servable route: every non-root path (i.e. `/admin`) then 404s. The build is single-entry, so `/admin` exists only via that rewrite.
