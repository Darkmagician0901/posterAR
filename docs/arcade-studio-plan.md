...
## Why this is a wiring job, not an engine rewrite

`src/story/eraArt.ts:4` records that today's era art is "the exact output of the design
prototype's scene builders (`sunflowerSVG`, `carSVG`, `fenceSVG`, `deadTreeSVG`, …),
extracted once to static files." Those functions are the prototype's prop library
(`arcade-studio-v4.html:502-723`). The coordinate systems already agree — shipped SVGs
are `viewBox="0 0 330 175"`, the studio composes at `0 0 330 200` with ground baseline
`GYB=141`.

The shipped art was already generated by this tool. We are re-connecting a pipeline that
was severed by a manual copy-paste, not inventing one.
...
| Stage editor, prop library, drag positioning | Kept | It is the era-art generator |
...
|---|---|---|
| **1 — Shared prop builders + schema** | Extract the prototype's builders into `src/story/props/` as pure functions; `StoryDoc` schema + validator + bundled default derived from `storyData.ts`; `contentStore` reads it with per-field fallback; viewer rewired to the store | Production story renders pixel-identically on device; regenerated SVGs diff-match `story/era/*.svg` |
| **2 — Studio UI** | `/studio` chunk: frames rail, inspector, stage editor, prop library, live phone preview, wash-color control; draft in `localStorage` with autosave + undo | Author a 3-frame story on desktop, see it in the studio's phone preview |
| **3 — Publish** | Vercel Blob adapter; `POST /api/publish`; `/?s=<id>` load path; link + QR poster; publish checks | Author on desktop, hit Publish, scan the QR on a phone, walk the story — no redeploy |
| **4 — Uploads + hardening** | Custom image props via the existing asset pipeline; delete-orphan fix; docs | |
