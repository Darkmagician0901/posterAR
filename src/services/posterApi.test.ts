  ...
  it('POSTs metadata then PUTs the bytes to the presigned url', async () => {
    const fetchMock = vi.fn()
      .mockResolvedValueOnce(new Response(JSON.stringify({
        ...
        asset: { id: 'id-1', url: 'https://pub/id-1.webp', contentType: 'image/webp',
                 isAnimated: false, width: 10, height: 20, originalName: 'a.webp' },
      }), { status: 201 }))
      .mockResolvedValueOnce(new Response(null, { status: 200 }))
    vi.stubGlobal('fetch', fetchMock)

    const asset = await persistAsset({
      id: 'id-1', blob: new Blob(['x']), contentType: 'image/webp',
      isAnimated: false, width: 10, height: 20, originalName: 'a.webp',
    })
...
describe('listAssets', () => {
  it('GETs the asset list for the owner', async () => {
    const fetchMock = vi.fn().mockResolvedValueOnce(new Response(JSON.stringify({
      assets: [{ id: 'a', url: 'u', contentType: 'image/webp', isAnimated: false,
                 width: 1, height: 1, originalName: null }],
    }), { status: 200 }))
    vi.stubGlobal('fetch', fetchMock)
