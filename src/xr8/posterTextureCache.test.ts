...
function makeResult(opts?: {
  ...
  const animator = opts?.animator !== undefined
    ? opts.animator
...
describe('animation byte budget', () => {
  ...
  it('passes 0 (or negative) remaining budget when the budget is exhausted', async () => {
    ...
    expect((secondCallArgs[1] as { animationByteBudget: number }).animationByteBudget).toBeLessThanOrEqual(0)
  })
  ...
  it('frees byte budget when the last reference to an animated entry is released', async () => {
    ...
    expect((thirdCallArgs[1] as { animationByteBudget: number }).animationByteBudget)
      .toBe(ANIMATION_BYTE_BUDGET - bigBytes)
  })
})
